...
st.text("   >  No data will be captured by this app and no machine learning algorithm was harmed during the making of this project :)")
...
st.text("   >  General words or stopwords may pop up because the accepted word list is still in a nascent stage, if they turn up a lot, please let me know via DM, I will filter them out for the next update")
...
if btn:
    ...
    stop_words.update(['high','related','complex','preferably','environment','code','hands','excellent','technical','familiarity','computer','one','best','use','paid','problems','identify','minimum','measure','large','datasets','closely','male','female','required','need','needs','science','engineer','engineering','ability','-','new','able','sets','provide','years','experience','he/she','responsibilities','working','good','strong','preferred','like','etc','experience','\uf0b7','Mr.','Ms',',','.','(',')','&','@','#','&','*',';',':','●','·','india','India',',','indian','experience','user','work','every','requirements','qualifications','using','–',',','’','—','•','/','%'])
    ...
    topjd_sum=sum([top_jd[x][1] for x in range(len(top_jd))])
    ...
    for word in [top_cv[x][0] for x in range(len(top_cv))]:
        if word in [top_jd[x][0] for x in range(len(top_jd))]:
            factor=float((cv_dict.get(word)/(jd_dict.get(word))/topjd_sum)*1000)
            score+=(factor*cv_dict.get(word))

    for word in [top_jd[x][0] for x in range(len(top_jd[:40]))]:
